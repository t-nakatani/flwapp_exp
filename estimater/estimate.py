...
def extract_patches(img_copy, patch_name, C, theta, path_patch):
    ...
    patch = cv2.resize(
        img_[cy - len_edge:cy + len_edge + 30, cx - len_edge - 5:cx + len_edge + 5], None, fx=0.5, fy=0.3333333
    )
    ...
    patch1 = img_[cy1_ - len_edge:cy1_ + len_edge, cx1_ - len_edge:cx1_ + len_edge]
    patch2 = img_[cy2_ - len_edge:cy2_ + len_edge, cx2_ - len_edge:cx2_ + len_edge]
    if cx1_ > cx2_:
...
def predict_and_create_patch_dic(path_patch):
    ...
    argparser.add_argument('--weight_path', type=str, help='path_weight', default='estimater/maml/model')
